...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen w-full flex bg-white font-sans text-slate-900">
      ...
      <div className="hidden lg:flex w-1/2 flex-col justify-between p-12 lg:p-20 relative overflow-hidden bg-[#fafafa] border-r border-slate-100">
        
        {/* Subtle mesh gradient background */}
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-gradient-to-bl from-orange-100 via-purple-50 to-transparent opacity-60 rounded-full blur-3xl pointer-events-none -translate-y-1/4 translate-x-1/4"></div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-white relative">
        <div className="w-full max-w-[400px] space-y-8">
          ...
          <div className="text-center space-y-2">
            <h2 className="text-3xl font-medium tracking-tight">Welcome to Tsenta</h2>
            <p className="text-slate-500 font-medium">Start with 25 free applications. No card required.</p>
          </div>

          <div className="space-y-4">
            <button 
              className="w-full flex items-center justify-center gap-3 h-12 rounded-full border border-slate-200 text-base font-medium shadow-sm hover:bg-slate-50 relative transition-colors disabled:opacity-50"
              onClick={handleGoogleLogin}
              disabled={isLoggingIn}
            >
              {isLoggingIn ? (
                <Loader2 className="w-5 h-5 animate-spin text-slate-400 absolute left-6" />
              ) : (
            ...
            <form onSubmit={handleEmailLogin} className="space-y-4 pt-2">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  required
                  className="w-full h-12 border-0 border-b-2 border-slate-300 rounded-none px-0 focus:outline-none focus:ring-0 focus:border-black bg-transparent text-base transition-colors" 
                />
              </div>
              <div className="space-y-2">
              ...
              <button 
                ...
                className="w-full h-12 rounded-full bg-black text-white hover:bg-slate-800 text-base font-medium transition-colors flex items-center justify-center disabled:opacity-50"
              >
                {isEmailLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Continue <span className="ml-2 font-light">›</span></>}
              </button>
            </form>
          </div>

          <div className="text-center space-y-5 pt-6">
            <button className="text-[13px] font-semibold text-slate-500 hover:text-slate-900 transition-colors underline decoration-slate-300 underline-offset-4">
              Sign in with messaging (WhatsApp / iMessage)
            </button>
            <p className="text-[11px] text-slate-400 font-medium">
              By continuing you agree to Tsenta's <a href="#" className="underline decoration-slate-300 hover:text-slate-600">Terms of Service</a> and <a href="#" className="underline decoration-slate-300 hover:text-slate-600">Privacy Policy</a>.
            </p>
          </div>
