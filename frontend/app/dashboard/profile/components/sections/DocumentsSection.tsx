...
export default function DocumentsSection({ latestResume, uploadMutation }: Props) {
  ...
  const handlePdfAction = async (action: 'preview' | 'download') => {
    ...
    try {
      ...
      if (action === 'preview') {
        // Cloudinary signed URLs can be opened directly. Since it has fl_attachment, 
        // some browsers might download it directly anyway, but opening in a new tab is safe.
