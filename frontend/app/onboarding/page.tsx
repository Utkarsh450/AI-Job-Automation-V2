...
const Step0 = ({ isUploading, file, referralCode, setReferralCode, handleFileUpload }: any) => (
  <div className="flex w-full max-w-[1200px] mx-auto bg-white dark:bg-[#141414] rounded-xl border border-slate-200 dark:border-white/10 shadow-2xl overflow-hidden min-h-[600px]">
    {/* Left Sidebar */}
    <div className="w-1/3 bg-slate-50 dark:bg-[#0f0f11] p-10 border-r border-slate-200 dark:border-white/5">
    ...
    <div className="w-2/3 p-12">
      ...
      <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-slate-200 dark:border-white/10 border-dashed rounded-xl cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5 transition-colors mb-10">
        <div className="flex flex-col items-center justify-center pt-5 pb-6">
          ...
          <p className="mb-2 text-sm text-slate-700 dark:text-slate-300">
            <span className="font-semibold text-emerald-600 dark:text-emerald-400">Drop your PDF here, or browse</span>
          </p>
      ...
      <div className="mb-8">
        ...
        <input 
          ...
          className="w-full bg-transparent border-b border-slate-200 dark:border-white/10 focus:border-emerald-500 py-2 outline-none text-slate-900 dark:text-white transition-colors"
          value={referralCode}
      ...
      <button 
        ...
        className="bg-[#111827] dark:bg-white text-white dark:text-[#111827] px-6 py-3 rounded-lg font-medium flex items-center disabled:opacity-50 hover:bg-slate-800 dark:hover:bg-slate-200 transition-colors"
      >
        {isUploading ? (
          <>
...
const MasterLayout = ({ children, stepNumber, totalSteps, step, setStep, isResumeParsed, handleFinalSubmit, isUploading, handleNextStep }: any) => (
  <div className="flex flex-col w-full max-w-[1200px] mx-auto bg-white dark:bg-[#141414] rounded-xl border border-slate-200 dark:border-white/10 shadow-2xl overflow-hidden min-h-[700px]">
    {/* Top Bar */}
    <div className="px-8 py-6 border-b border-slate-200 dark:border-white/5">
      ...
      <div className="w-full bg-slate-100 dark:bg-[#111] h-1 rounded-full overflow-hidden">
    ...
    <div className="flex flex-1">
      {/* Left Sidebar */}
      <div className="w-64 bg-slate-50 dark:bg-[#0f0f11] p-8 border-r border-slate-200 dark:border-white/5 shrink-0">
        <div className="mb-10">
          ...
          {!isResumeParsed ? (
             <div className="w-full bg-slate-200 dark:bg-[#222] h-1 rounded-full overflow-hidden mb-4">
      ...
      <div className="flex-1 p-10 flex flex-col">
        ...
        <div className="mt-10 flex items-center">
          <button 
            ...
            disabled={isUploading}
            className="bg-[#111827] dark:bg-white text-white dark:text-[#111827] px-6 py-3 rounded-lg font-medium flex items-center hover:bg-slate-800 dark:hover:bg-slate-200 transition-colors disabled:opacity-50"
          >
            {isUploading ? (
              <>
                <Loader2 className="w-4 h-4 mr-2 animate-spin" /> {step === totalSteps ? 'Saving...' : 'Loading...'}
              </>
            ) : (
              <>
...
export default function OnboardingWizard() {
  ...
  const handleNextStep = async () => {
    // Validation
    if (step === 1) {
      if (!location.address || !location.city || !location.zip || !location.country || !location.state) {
        alert("Please fill out all location fields to continue.");
        return;
      }
    } else if (step === 2) {
      if (isAuthorized === null || needsSponsorship === null || locatedInUS === null) {
        alert("Please answer all work eligibility questions to continue.");
        return;
      }
    } else if (step === 3) {
      if (!preferences.targetSalary) {
        alert("Please specify a target salary to continue.");
        return;
      }
      if (preferences.enrolledInPhD && !preferences.yearsInPhD) {
        alert("Please specify years in your Ph.D program.");
        return;
      }
    } else if (step === 5) {
      if (appPasswords.some(p => !p.password)) {
        alert("Please set an application password to continue.");
        return;
      }
    }
    
    // Show actual loader for intermediate steps
    setIsUploading(true);
    await new Promise(resolve => setTimeout(resolve, 600)); // 600ms loader simulation
    setIsUploading(false);
    setStep(step + 1);
  };

  return (
    <div className="min-h-screen w-full bg-slate-100 dark:bg-[#0f0f11] flex items-center justify-center p-4 py-12">
      {step === 0 && (
        <Step0 
      ...
      {step === 1 && (
        <MasterLayout stepNumber={1} totalSteps={6} step={step} setStep={setStep} isResumeParsed={isResumeParsed} handleFinalSubmit={handleFinalSubmit} isUploading={isUploading} handleNextStep={handleNextStep}>
          ...
          <div className="mb-8">
            ...
            <input 
              ...
              className="w-full bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-lg px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 text-slate-900 dark:text-white text-lg shadow-sm transition-shadow"
          ...
          <div className="grid grid-cols-2 gap-6 mb-8">
            <div>
              ...
              <input type="text" placeholder="San Francisco" className="w-full bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-lg px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 text-slate-900 dark:text-white shadow-sm transition-shadow"
            ...
            <div>
              ...
              <input type="text" placeholder="94103" className="w-full bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-lg px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 text-slate-900 dark:text-white shadow-sm transition-shadow"
            ...
            <div>
              ...
              <input type="text" placeholder="United States" className="w-full bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-lg px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 text-slate-900 dark:text-white shadow-sm transition-shadow"
            ...
            <div>
              ...
              <input type="text" placeholder="CA" className="w-full bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-lg px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 text-slate-900 dark:text-white shadow-sm transition-shadow"
      ...
      {step === 2 && (
        <MasterLayout stepNumber={2} totalSteps={6} step={step} setStep={setStep} isResumeParsed={isResumeParsed} handleFinalSubmit={handleFinalSubmit} isUploading={isUploading} handleNextStep={handleNextStep}>
          ...
          <div className="grid grid-cols-3 gap-3 mb-10">
            {['US Citizen', 'Permanent Resident', 'H-1B', 'F-1 (Student)', 'OPT', 'CPT', 'J-1', 'L-1', 'O-1', 'TN', 'E-3', 'Other'].map(status => (
              <button 
                ...
                className={`py-3 px-4 rounded-lg border text-sm font-medium transition-all text-left ${visaStatus === status ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 ring-1 ring-emerald-500' : 'border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-white/20'}`}
              >
                {status}
              </button>
            ))}
          ...
          <div className="space-y-6 pt-6 border-t border-slate-100 dark:border-[#222]">
            <div className="flex justify-between items-center">
              ...
              <div className="flex bg-slate-100 dark:bg-[#111] rounded-lg p-1">
                <button onClick={() => setIsAuthorized(true)} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all active:scale-95 ${isAuthorized === true ? 'bg-white dark:bg-[#222] text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>Yes</button>
                <button onClick={() => setIsAuthorized(false)} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all active:scale-95 ${isAuthorized === false ? 'bg-white dark:bg-[#222] text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>No</button>
              </div>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-slate-700 dark:text-slate-300">Will you now, or in the future, require sponsorship?</span>
              <div className="flex bg-slate-100 dark:bg-[#111] rounded-lg p-1">
                <button onClick={() => setNeedsSponsorship(true)} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all active:scale-95 ${needsSponsorship === true ? 'bg-white dark:bg-[#222] text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>Yes</button>
                <button onClick={() => setNeedsSponsorship(false)} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all active:scale-95 ${needsSponsorship === false ? 'bg-white dark:bg-[#222] text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>No</button>
              </div>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-slate-700 dark:text-slate-300">Are you currently located in the US?</span>
              <div className="flex bg-slate-100 dark:bg-[#111] rounded-lg p-1">
                <button onClick={() => setLocatedInUS(true)} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all active:scale-95 ${locatedInUS === true ? 'bg-white dark:bg-[#222] text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>Yes</button>
                <button onClick={() => setLocatedInUS(false)} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all active:scale-95 ${locatedInUS === false ? 'bg-white dark:bg-[#222] text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>No</button>
              </div>
            </div>
          </div>
        </MasterLayout>
      ...
      {step === 3 && (
        <MasterLayout stepNumber={3} totalSteps={6} step={step} setStep={setStep} isResumeParsed={isResumeParsed} handleFinalSubmit={handleFinalSubmit} isUploading={isUploading} handleNextStep={handleNextStep}>
          ...
          <div className="border border-slate-200 dark:border-white/10 rounded-xl overflow-hidden divide-y divide-slate-100 dark:divide-[#222]">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-6 hover:bg-slate-50/50 dark:hover:bg-[#1a1a1a]/50 gap-4">
              ...
              <div className="relative group">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-medium group-focus-within:text-emerald-500 transition-colors duration-300">$</span>
                <input 
                  type="number" 
                  placeholder="120000"
                  className="w-full sm:w-48 bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-lg pl-8 pr-4 py-2 outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 text-slate-900 dark:text-white shadow-sm transition-all duration-300 hover:border-emerald-300 dark:hover:border-emerald-700"
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-6 hover:bg-slate-50/50 dark:hover:bg-[#1a1a1a]/50 gap-4 border-t border-slate-100 dark:border-[#222]">
              ...
              <input 
                ...
                className="w-full sm:w-64 bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 text-slate-900 dark:text-white shadow-sm transition-all duration-300 hover:border-emerald-300 dark:hover:border-emerald-700"
                value={linkedinUrl}
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-6 hover:bg-slate-50/50 dark:hover:bg-[#1a1a1a]/50 gap-4 border-t border-slate-100 dark:border-[#222]">
              ...
              <div className="flex bg-slate-100 dark:bg-[#111] rounded-lg p-1 overflow-x-auto w-full sm:w-auto shadow-inner">
            ...
            <div className="flex justify-between items-center p-6 hover:bg-slate-50/50 dark:hover:bg-[#1a1a1a]/50">
              ...
              <div className="flex bg-slate-100 dark:bg-[#111] rounded-lg p-1">
                <button onClick={() => setPreferences({...preferences, foreignTies: true})} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all active:scale-95 ${preferences.foreignTies ? 'bg-[#111827] dark:bg-white text-white dark:text-[#111827] shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>Yes</button>
                <button onClick={() => setPreferences({...preferences, foreignTies: false})} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all active:scale-95 ${!preferences.foreignTies ? 'bg-[#111827] dark:bg-white text-white dark:text-[#111827] shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>No</button>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center p-6 hover:bg-slate-50/50 dark:hover:bg-[#1a1a1a]/50">
              ...
              <div className="flex bg-slate-100 dark:bg-[#111] rounded-lg p-1 shrink-0 ml-4">
                <button onClick={() => setPreferences({...preferences, previouslyEmployed: true})} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all active:scale-95 ${preferences.previouslyEmployed ? 'bg-[#111827] dark:bg-white text-white dark:text-[#111827] shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>Yes</button>
                <button onClick={() => setPreferences({...preferences, previouslyEmployed: false})} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all active:scale-95 ${!preferences.previouslyEmployed ? 'bg-[#111827] dark:bg-white text-white dark:text-[#111827] shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>No</button>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center p-6 hover:bg-slate-50/50 dark:hover:bg-[#1a1a1a]/50 border-t border-slate-100 dark:border-[#222]">
              ...
              <div className="flex bg-slate-100 dark:bg-[#111] rounded-lg p-1 shrink-0 ml-4">
                <button onClick={() => setPreferences({...preferences, enrolledInPhD: true})} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all active:scale-95 ${preferences.enrolledInPhD ? 'bg-[#111827] dark:bg-white text-white dark:text-[#111827] shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>Yes</button>
                <button onClick={() => setPreferences({...preferences, enrolledInPhD: false})} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all active:scale-95 ${!preferences.enrolledInPhD ? 'bg-[#111827] dark:bg-white text-white dark:text-[#111827] shadow-sm' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10'}`}>No</button>
              </div>
            </div>
      ...
      {step === 4 && (
        <MasterLayout stepNumber={4} totalSteps={6} step={step} setStep={setStep} isResumeParsed={isResumeParsed} handleFinalSubmit={handleFinalSubmit} isUploading={isUploading} handleNextStep={handleNextStep}>
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {['Male', 'Female', 'Non-binary', 'Decline'].map(opt => (
                  <button 
                    ...
                    className={`py-3 px-4 rounded-xl border text-sm font-medium transition-all ${demographics.gender === opt ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 ring-1 ring-emerald-500 shadow-sm' : 'border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-white/20 bg-white dark:bg-[#111]'}`}
                  >
                    {opt}
                  </button>
                ))}
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {['she/her', 'he/him', 'they/them', 'Decline'].map(opt => (
                  <button 
                    ...
                    className={`py-3 px-4 rounded-xl border text-sm font-medium transition-all ${demographics.pronouns === opt ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 ring-1 ring-emerald-500 shadow-sm' : 'border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-white/20 bg-white dark:bg-[#111]'}`}
                  >
                    {opt}
                  </button>
                ))}
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                {['Asian', 'Black', 'Hispanic', 'White', 'Other', 'Decline'].map(opt => (
                  <button 
                    ...
                    className={`py-3 px-4 rounded-xl border text-sm font-medium transition-all ${demographics.race === opt ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 ring-1 ring-emerald-500 shadow-sm' : 'border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-white/20 bg-white dark:bg-[#111]'}`}
                  >
                    {opt}
                  </button>
                ))}
              ...
              <div className="flex flex-col gap-3">
                {['I am not a protected veteran', 'I am a protected veteran', 'Decline'].map(opt => (
                  <button 
                    ...
                    className={`py-3 px-4 rounded-xl border text-sm font-medium transition-all text-left flex items-center ${demographics.veteranStatus === opt ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 ring-1 ring-emerald-500 shadow-sm' : 'border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-white/20 bg-white dark:bg-[#111]'}`}
                  >
                    <div className={`w-4 h-4 rounded-full border mr-3 flex items-center justify-center ${demographics.veteranStatus === opt ? 'border-emerald-600 dark:border-emerald-400' : 'border-slate-300 dark:border-[#555]'}`}>
                      {demographics.veteranStatus === opt && <div className="w-2 h-2 rounded-full bg-emerald-600 dark:bg-emerald-400" />}
                    </div>
                    {opt}
                  </button>
                ))}
            ...
            <div>
              ...
              <div className="flex bg-slate-100 dark:bg-[#111] rounded-xl p-1.5 w-full sm:w-fit">
                {['No', 'Yes', 'Decline'].map(opt => (
                  <button 
                    key={opt} onClick={() => setDemographics({...demographics, disabilityStatus: opt})}
                    className={`px-6 py-2 rounded-lg text-sm font-medium transition-all ${demographics.disabilityStatus === opt ? 'bg-white dark:bg-[#222] text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                  >
                    {opt}
                  </button>
                ))}
              ...
              <div className="flex bg-slate-100 dark:bg-[#111] rounded-xl p-1.5 w-full sm:w-fit">
                {['No', 'Yes', 'Decline'].map(opt => (
                  <button 
                    ...
                    className={`px-6 py-2 rounded-lg text-sm font-medium transition-all active:scale-95 ${demographics.lgbtqStatus === opt ? 'bg-white dark:bg-[#222] text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                  >
                    {opt}
                  </button>
                ))}
      ...
      {step === 5 && (
        <MasterLayout stepNumber={5} totalSteps={6} step={step} setStep={setStep} isResumeParsed={isResumeParsed} handleFinalSubmit={handleFinalSubmit} isUploading={isUploading} handleNextStep={handleNextStep}>
          ...
          <div className="flex space-x-2 mb-8">
            <span className="px-3 py-1 rounded-full bg-slate-100 dark:bg-[#111] text-xs font-medium text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-white/10">Workday</span>
            <span className="px-3 py-1 rounded-full bg-slate-100 dark:bg-[#111] text-xs font-medium text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-white/10">iCIMS</span>
            <span className="px-3 py-1 rounded-full bg-slate-100 dark:bg-[#111] text-xs font-medium text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-white/10">Oracle</span>
            <span className="px-3 py-1 text-xs font-medium text-slate-400 self-center">+ more</span>
          </div>

          <div className="border border-slate-200 dark:border-white/10 rounded-xl overflow-hidden mb-6">
            <div className="bg-slate-50 dark:bg-[#121212] px-6 py-3 flex justify-between items-center border-b border-slate-200 dark:border-white/10">
              <span className="text-xs font-bold tracking-wider text-slate-400 uppercase">Password</span>
          <button className="text-xs font-medium text-emerald-600 dark:text-emerald-400 hover:underline" onClick={generatePassword}>
            ...
            <div className="p-6">
              <div className="relative mb-6">
                <input 
                  ...
                  className="w-full bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-lg px-4 py-3 pr-10 outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 text-slate-900 dark:text-white shadow-sm transition-shadow"
      ...
      {step === 6 && (
        <MasterLayout stepNumber={6} totalSteps={6} step={step} setStep={setStep} isResumeParsed={isResumeParsed} handleFinalSubmit={handleFinalSubmit} isUploading={isUploading} handleNextStep={handleNextStep}>
          ...
          <div className="border border-slate-200 dark:border-white/10 rounded-xl overflow-hidden mb-6 divide-y divide-slate-100 dark:divide-[#222]">
            <div className="bg-slate-50 dark:bg-[#121212] px-6 py-3">
              <span className="text-xs font-bold tracking-wider text-slate-400 uppercase">Resume Optimization</span>
            </div>
            
            <div className={`p-5 cursor-pointer flex items-start transition-colors ${resumeOptimization === 'Off' ? 'bg-emerald-50 dark:bg-blue-900/10' : 'hover:bg-slate-50/50 dark:hover:bg-[#1a1a1a]/50'}`} onClick={() => setResumeOptimization('Off')}>
            ...
            <div className={`p-5 cursor-pointer flex items-start transition-colors ${resumeOptimization === 'Honest' ? 'bg-slate-100 dark:bg-[#111]' : 'hover:bg-slate-50/50 dark:hover:bg-[#1a1a1a]/50'}`} onClick={() => setResumeOptimization('Honest')}>
            ...
            <div className={`p-5 cursor-pointer flex items-start transition-colors ${resumeOptimization === 'Aggressive' ? 'bg-slate-100 dark:bg-[#111]' : 'hover:bg-slate-50/50 dark:hover:bg-[#1a1a1a]/50'}`} onClick={() => setResumeOptimization('Aggressive')}>
